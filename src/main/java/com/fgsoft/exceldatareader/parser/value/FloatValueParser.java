...
/**
 * Single cell value parser in charge of returning a Float. The Float to be returned can be defined
 ...
 * </ul>
 */
public class FloatValueParser extends AbstractSingleCellValueParser<Float> {
    @Override
    protected Float getValueForEmptyCell(int rowIndex, int colIndex, Sheet worksheet) {
        return null;
    }

    @Override
    protected Float getValueForCell(double value, int rowIndex, int colIndex, Sheet worksheet) {
        return (float) value;
    }

    @Override
    protected Float getValueForCell(boolean value, int rowIndex, int colIndex, Sheet worksheet) {
        throw new IncorrectValueForTypeException(null, value, Float.class.getName(),
                rowIndex, colIndex, worksheet.getSheetName());
    }

    @Override
    protected Float getValueForCell(String value, int rowIndex, int colIndex, Sheet worksheet) {
        final float retVal;
        try {
