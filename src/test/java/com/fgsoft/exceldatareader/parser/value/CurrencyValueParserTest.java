...
class CurrencyValueParserTest {
    ...
    private final CurrencyValueParser parser = new CurrencyValueParser();

    @Test
    final void testParseStringValueOK() {
        // Given
        final Currency expected = Currency.getInstance("EUR");
        when(cell.getStringCellValue()).thenReturn("EUR");
        when(cell.getCellType()).thenReturn(CellType.STRING);
        // When
        final Currency value = parser.getValue(cell, evaluator);
        // Then
    ...
    final void testParseStringValueKO() {
        ...
        final String message = String.format(ExcelReaderErrorCode.INCORRECT_VALUE_FOR_TYPE.getMessage(),
                strValue, Currency.class.getName(), 0, 0, SHEET_NAME);
        // When
    ...
    final void testDoubleValue() {
        ...
        when(cell.getColumnIndex()).thenReturn(0);
        final String message = String.format(ExcelReaderErrorCode.INCORRECT_VALUE_FOR_TYPE.getMessage(),
                dblValue, Currency.class.getName(), 0, 0, SHEET_NAME);
        // When
        Throwable exception = assertThrows(ExcelReaderException.class,
                () -> parser.getValue(cell, evaluator));
        // Then
    ...
    final void testBooleanValueOK() {
        ...
        when(cell.getColumnIndex()).thenReturn(0);
        final String message = String.format(ExcelReaderErrorCode.INCORRECT_VALUE_FOR_TYPE.getMessage(),
                boolValue, Currency.class.getName(), 0, 0, SHEET_NAME);
        // When
        Throwable exception = assertThrows(ExcelReaderException.class,
                () -> parser.getValue(cell, evaluator));
        // Then
    ...
    final void testNullValueOK() {
        // Given
        // When
        final Currency value = parser.getValue(null, evaluator);
        // Then
        assertThat(value).isNull();
    }
    ...
    final void testFormulaReturningABoolean() {
        ...
        when(cell.getColumnIndex()).thenReturn(0);
        final String message = String.format(ExcelReaderErrorCode.INCORRECT_VALUE_FOR_TYPE.getMessage(),
                Boolean.TRUE, Currency.class.getName(), 0, 0, SHEET_NAME);
        // When
        Throwable exception = assertThrows(ExcelReaderException.class,
                () -> parser.getValue(cell, evaluator));
        // Then
